...
if [[ "$BOOTNODE" == "" ]];then
    BOOTNODE="cnode://733edf2d1cb2faa843f71c54e0ab9fff676b6f3e937cefb44522d9acdfaf3a0208290a0b7d6aa58398412b95de86230c9ddfffe3af6e5e19455e7be9d9c458bd@18.237.213.2:30301"
fi
...
append_node()
{
    ...
    for m in $( seq $NODES_NUM ); do
        ...
        $CYPHER  --onetport 7000 --nat "none" --ws --tps -wsaddr="0.0.0.0" --wsorigins "*" --rpc --rpccorsdomain "*" --rpcaddr 0.0.0.0 --rpcapi eth,web3,personal,miner --port $((16000 + 2 * $n)) --rpcport $((18000 + 2 * $n)) --verbosity "$VERBOSITY" --datadir "$NODE_DIR/$n" --networkid "$NETWORKID" --gcmode archive --bootnodes "$BOOTNODE" 2>"$NODE_DIR/$n/output.log"
    done
    popd
}
