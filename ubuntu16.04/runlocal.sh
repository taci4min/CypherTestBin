...
append_node()
{
    ...
    for m in $( seq $NODES_NUM ); do
        ...
        $CYPHER  --onetport 7000 --nat "none" --ws --tps -wsaddr="0.0.0.0" --wsorigins "*" --rpc --rpccorsdomain "*" --rpcaddr 0.0.0.0 --rpcapi eth,web3,personal --port $((16000 + 2 * $n)) --rpcport $((18000 + 2 * $n)) --verbosity 6 --datadir "$NODE_DIR/$n" --networkid 123666 --gcmode archive --bootnodes "$BOOTNODE" 2>"$NODE_DIR/$n/output.log" &
    done
